...
async function sendRequestWithRetry(temperature: number, model: string, systemPrompt: string, userPrompt: string, retries = 3, delay = 3000) { // Increased initial delay
  let lastError = null;
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error('Request failed:', lastError.response || error);
...
export async function strict_output(
  ...
  for (let i = 0; i < num_tries; i++) {
    ...
    const systemPrompt = system_prompt + output_format_prompt + error_msg;
